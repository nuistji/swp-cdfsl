...
class SimpleBlock(nn.Module):
    def __init__(self, method, indim, outdim, half_res, track_bn):
        ...
        # self.C1 = nn.Conv1d(indim, outdim, kernel_size=1, stride=2 if half_res else 1, padding=1, bias=False)
        ...
        # self.C3 = nn.Conv2d(outdim, outdim, kernel_size=1, stride=1 if half_res else 1, padding=0, bias=False)
        # self.BN3 = nn.BatchNorm2d(outdim, track_running_stats=track_bn)
        #
        self.relu1 = nn.ReLU(inplace=True)
        self.relu2 = nn.ReLU(inplace=True)

        # self.parametrized_layers = [self.C1, self.C2, self.BN1, self.BN2, self.C3, self.BN3]
