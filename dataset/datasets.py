...
class ISICDataset(ImageFolder):
    ...
    def __init__(self, root=ISIC_path, *args, **kwargs):
        ...
        # self.metadata = pd.read_csv(csv_path, skiprows=[0], header=None)
        self.metadata = pd.read_csv(csv_path)

        root = os.path.join(root, "ISIC2018_Task3_Training_Input")
        # samples = self.make_dataset(root, *args, **kwargs)
        super().__init__(root, *args, **kwargs)
